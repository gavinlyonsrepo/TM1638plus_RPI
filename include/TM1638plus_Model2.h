...
* Description: TM1638plus_Model2.h header file RPI library 
* for Model 2 TM1638 module(16 KEY 16 pushbuttons).
* Author: Gavin Lyons.
* Created June 2021
...
public:
	...
	// 4. swap_nibbles = bool default false, if true swaps nibbles on display uint8_t
	...
	// Returns: a uint8_t with decimal value of buttons 1-16 (1 for S1 2 for S2 ... 16 for S16 ).
   uint8_t ReadKey16(void);
	...
	// pass segment  0-7, uint8_t of data corresponding to segments abcdefg(dp) 01234567.
	...
	// Then passes array of eight ASCII uint8_ts to DisplayValues function
	void DisplayStr(const char* string, uint16_t dots = 0);
   
	// Takes in Array of 8 ASCII uint8_ts , Called from DisplayStr 
	// Scans each ASCII uint8_t converts to array of 8 segment uint8_ts where each uint8_t represents a segment
	// i.e. uint8_t 0 represents a in segment and then each bit represents the a segment in each digit.
	...
	// So "a" segment is turned on for all uint8_ts and "b" is on for all except last digit.
